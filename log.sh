...
if [ "$contest"  -eq 1 ]
then
  echo "Callsign,Frequency,Mode,TX-RST,TX-SER,RX-RST,RX-SER,UTC-On,UTC-Off,Operator,Station,Date,Power,Locator" >> contest_log_$filename\_$date.csv
  declare -i txser=1
elif [ "$contest" -ne 1 ]
then
  echo "Callsign,Frequency,Mode,TX-RST,RX-RST,UTC-On,UTC-Off,Operator,Station,Date,Power,Locator" >> log_$filename\_$date.csv
fi
...
then
  ...
  if [ "$contest" -eq 1 ]
  then
    echo "$call,$freq,$mode,$tx,$txser,$rx,$rxser,$utcon,$utcoff,$op,$station,$date,$power,$locator" >> contest_log_$filename\_$date.csv
    txser=$((txser+1))
  elif [ "$contest" -ne 1 ]
  then
    echo "$call,$freq,$mode,$tx,$rx,$utcon,$utcoff,$op,$station,$date,$power,$locator" >> log_$filename\_$date.csv
  fi
